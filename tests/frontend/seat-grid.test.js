...
const { gridSelectionPatch, shrinkGridPatch, seatGridRows } =
  loadFunctions(['gridSelectionPatch', 'shrinkGridPatch', 'seatGridRows'], { SEAT_GRID_MAX: 6 });
...
test('卓を減らしたら、盤面の行数も必要な数まで詰める', () => {
  // 4卓を 2行×2列 にしていた次会から、卓を2つ削除した
  assert.deepEqual(shrinkGridPatch({ tableRows: 2, tableCols: 2 }, 2), { tableRows: 1 });
});

test('必要な行数と同じなら書き込まない', () => {
  assert.equal(shrinkGridPatch({ tableRows: 2, tableCols: 2 }, 4), null);
});

test('必要な行数より小さければ書き込まない（seatGridRows が広げる）', () => {
  assert.equal(shrinkGridPatch({ tableRows: 1, tableCols: 2 }, 4), null);
});

test('行数が未設定の次会は触らない', () => {
  assert.equal(shrinkGridPatch({ tableCols: 2 }, 2), null);
});

test('配置を調整した次会(freeLayout)は詰めない（意図して空けた配置を壊さない）', () => {
  assert.equal(shrinkGridPatch({ tableRows: 3, tableCols: 2, freeLayout: true }, 2), null);
});

test('卓が0になっても行は1を下回らない', () => {
  assert.deepEqual(shrinkGridPatch({ tableRows: 4, tableCols: 2 }, 0), { tableRows: 1 });
});
...
test('報告された手順：4卓で列を2にし、2卓へ減らしても空きマスが出ない', () => {
  ...
  Object.assign(party, shrinkGridPatch(party, 2) || {});
