...
test('isRetryableError: 候補レベルの SAFETY（finishReason=SAFETY）も再試行しない', () => {
  assert.equal(isRetryableError(new Error('gemini: empty parts text (finishReason=SAFETY)')), false);
});

test('isRetryableError: 個別タイムアウト（GEMINI_TIMEOUT）は一時的エラーとして再試行対象', () => {
  assert.equal(isRetryableError(new Error('GEMINI_TIMEOUT: no response in 25s')), true);
});
