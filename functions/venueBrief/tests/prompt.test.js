...
test('systemInstruction: プロンプト強化（検索の多観点化・情報源の厳格化）を含む', () => {
  const body = buildGeminiRequestBody('X', '');
  const sys = body.systemInstruction.parts[0].text;
  assert.ok(sys.includes('最低3回'), '検索の多観点化');
  assert.ok(sys.includes('口コミ'), '情報源の言及');
});

test('systemInstruction: 検索網は食べログに限らず幅広い情報源を使う', () => {
  const body = buildGeminiRequestBody('X', '');
  const sys = body.systemInstruction.parts[0].text;
  assert.ok(sys.includes('公式サイト'), '公式サイトの言及');
  assert.ok(sys.includes('Googleマップ'), 'Googleマップレビューの言及');
  assert.ok(sys.includes('インタビュー'), 'メディア記事の言及');
  // シェフの経歴・受賞歴など詳しい情報の収集指示
  assert.ok(sys.includes('受賞歴'), '受賞歴・掲載情報の言及');
});
