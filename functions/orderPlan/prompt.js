...
- プランとは別に、余裕があれば追加で頼みたいおすすめメニューを 2品程度（1〜3品）extras に挙げる。
  プランに含めた品・除外リストの品と重複させない。これも実在確認できたメニューのみ。
  extras は totalEstimate（合計目安）には含めない。

【出力形式】
- 出力は必ず有効な JSON オブジェクト1つのみ。コードフェンスや前置きは一切含めない。
- 応答の1文字目は { 、最終文字は } でなければならない。

{"shopFound": true, "plan": [{"category": "前菜", "items": [{"name": "品名", "qty": 2, "price": "¥500前後", "why": "一言理由"}]}], "totalEstimate": "¥3,500/人 前後", "notes": "補足（任意）", "extras": [{"name": "品名", "price": "¥600前後", "why": "一言理由"}]}`;
...
function parseOrderPlanResponse(apiJson) {
  ...
  return {
    ...
    // 追加のおすすめ（プラン外・最大3件）。name の無い要素は捨てるベストエフォート扱いで、
    // extras の欠落や乱れでプラン全体を失敗（リトライ）させない
    extras: Array.isArray(obj.extras) ? obj.extras
      .map(e => ({
        name: String((e && e.name) || '').trim(),
        price: String((e && e.price) || '').trim(),
